<template>
  <el-card class="general-monitor-page" shadow="never">
    <!-- 头部 -->
    <el-card class="search-bar" shadow="never">
      <el-form :model="queryParams" :inline="true" label-width="80px">
        <el-row>
          ...
          <el-col :span="5">
            <el-form-item>
              <el-button class="search-btn" type="primary" @click="handleQuery">搜索</el-button>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-card class="main-content" v-loading="loading" shadow="never">
      ...
      <div class="chart-box">
        ...
        <div v-if="xBchartData.length == 0">暂无数据</div>
        <basic-line-chart
          v-if="xBchartData.length > 0"
          :xb-chart-data="xBchartData"
          :yb-chart-data="yBchartData"
          :time-type="timeType"
        />
      </div>
    </el-card>
...
export default {
  ...
  methods: {
    async searchAppOptionList() {
      let resp = await queryOpenAppList({ pageNo: 1, pageSize: 999999,admin:this.user.user_type === 'super' });
      if (resp.code === "200") {
      ...
      getChartStatistics(data).then((res) => {
        if (res.data) {
          ...
          res.data.forEach((item) => {
            this.$nextTick(() => {
              this.xBchartData.push(item.date);
              this.xSchartData.push(item.date);
              this.yBchartData.push(item.useTime);
              this.ySchartData.push(item.callCount);
            });
          });
        }
      });
